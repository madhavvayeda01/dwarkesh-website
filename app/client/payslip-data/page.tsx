...
export default function ClientPayslipDataPage() {
  ...
  async function loadFiles() {
    setLoading(true);
    const res = await fetch("/api/client/payslip/records", { cache: "no-store" });
    const data = await res.json().catch(() => ({}));
    const payload = data?.data ?? data;
    if (!res.ok) {
      setFiles([]);
      setStatus(data?.message || "Failed to fetch payslip files.");
      setLoading(false);
      return;
    }
  ...
  async function deleteFile(fileName: string) {
    const confirmed = window.confirm(`Delete payslip file "${fileName}"?`);
    if (!confirmed) return;
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      setStatus(data?.message || "Failed to delete payslip file.");
      return;
    }

    setStatus("Payslip file deleted.");
    await loadFiles();
  }
        ...
        {moduleEnabled === false ? (
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-blue-950">Payslip Data</h1>
                <p className="mt-1 text-slate-600">
                  Generated payslip files are saved here and can be exported.
                </p>
              </div>
            ...
            <div className="mt-6 rounded-2xl bg-white p-6 shadow">
              {loading ? (
                <p className="text-sm text-slate-600">Loading payslip files...</p>
              ) : files.length === 0 ? (
                <p className="text-sm text-slate-600">No payslip files found yet.</p>
              ) : (
