...
export default function ClientAdvanceDataPage() {
  const [files, setFiles] = useState<AdvanceFile[]>([]);
  ...
  async function loadFiles() {
    setLoading(true);
    const res = await fetch("/api/client/advance/records", { cache: "no-store" });
  ...
  async function deleteFile(fileName: string) {
    const confirmed = window.confirm(`Delete advance file "${fileName}"?`);
    if (!confirmed) return;
    ...
    if (!res.ok) {
      setStatus(data?.message || "Failed to delete advance file.");
      return;
    }

    setStatus("Advance file deleted.");
    await loadFiles();
  }
        ...
        {moduleEnabled === false ? (
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-blue-950">Advance Data</h1>
                <p className="mt-1 text-slate-600">
                  Generated advance files are saved here and can be exported.
                </p>
              </div>
            ...
            <div className="mt-6 rounded-2xl bg-white p-6 shadow">
              {loading ? (
                <p className="text-sm text-slate-600">Loading advance files...</p>
              ) : files.length === 0 ? (
                <div className="space-y-3">
                  <p className="text-sm text-slate-600">
                    No advance files found for this client yet.
                  </p>
                  <a
                    href="/client/advance"
