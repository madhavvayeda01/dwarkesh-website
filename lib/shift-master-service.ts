import { EmployeeShiftCategory, WeekendType } from "@prisma/client";
...
type EmployeeContext = {
  ...
  otHoursTarget: number;
  gender: string | null;
  shiftCategory: EmployeeShiftCategory;
  shift: ShiftKey;
  weeklyOffDay: number;
  baseSeed: number;
};
...
type ShiftMasterConfigNormalized = ShiftMasterConfigInput & {
  warnings: string[];
};
...
type LoadErrorResult = {
  error: string;
  warnings?: GenerationWarning[];
  missingEmployeePayroll?: Array<{
    employeeId: string;
    empCode: string;
    employeeName: string;
  }>;
...
function hasLoadError(value: unknown): value is LoadErrorResult {
  if (!value || typeof value !== "object") return false;
  return typeof (value as { error?: unknown }).error === "string";
}

export function normalizeGender(value: string | null | undefined) {
  const normalized = toUpper(value);
  if (normalized === "F" || normalized === "FEMALE") return "F" as const;
  if (normalized === "M" || normalized === "MALE") return "M" as const;
  return null;
}

export function normalizeShiftCategory(
  value: EmployeeShiftCategory | string | null | undefined
): EmployeeShiftCategory {
  const normalized = toUpper(typeof value === "string" ? value : String(value || ""));
  return normalized === "STAFF" ? "STAFF" : "WORKER";
}

export function normalizeShiftMasterConfigInput(
  payload: ShiftMasterConfigInput
): ShiftMasterConfigNormalized {
  const warnings: string[] = [];
  if (!payload.generalShiftEnabled) {
    warnings.push("General shift (G) is mandatory and was force-enabled.");
  }
...
async function loadShiftConfig(clientId: string): Promise<ShiftConfig | null> {
  ...
  const enabledShifts: ShiftKey[] = ["G"];
  if (row.shiftAEnabled) enabledShifts.push("A");
  if (row.shiftBEnabled) enabledShifts.push("B");
  if (row.shiftCEnabled) enabledShifts.push("C");
...
export function resolveEmployeeShift(
  ...
  if (isFemale(params.gender)) return "G";
  if (params.shiftCategory === "STAFF") return "G";

  const workerShifts = params.enabledShifts.filter((shift): shift is "A" | "B" | "C" => shift !== "G");
  if (workerShifts.length === 0) return "G";

  const rng = createSeededRandom(
    hashSeed(`${params.clientId}|${params.employeeId}|${params.month}|${params.year}|shift`)
  );
  return workerShifts[Math.floor(rng() * workerShifts.length)] || workerShifts[0];
}

export function resolveWeeklyOffDay(
  params: {
    clientId: string;
    employeeId: string;
    month: number;
  ...
  if (params.weekendType !== "ROTATIONAL") {
    return fixedWeekendToDay(params.weekendType);
  }

  const rng = createSeededRandom(
    hashSeed(`${params.clientId}|${params.employeeId}|${params.month}|${params.year}|weekoff`)
  );
...
function buildRecordsForEmployee(
  ...
  const shiftStart = shiftConfig.shifts[employee.shift].start;
...
export async function loadInOutGeneratorInputs(clientId: string, month: number, year: number) {
  const [shiftConfig, holidays, payrollRows, employees] = await Promise.all([
    ...
    prisma.employee.findMany({
      where: { clientId },
      select: { id: true, empNo: true, fullName: true, gender: true, shiftCategory: true },
  ...
  for (const payroll of payrollRows) {
    ...
    const shiftCategory = normalizeShiftCategory(employee.shiftCategory);
    const shift = resolveEmployeeShift({
      ...
      shiftCategory,
      enabledShifts: shiftConfig.enabledShifts,
    });
    const weeklyOffDay = resolveWeeklyOffDay({
      clientId,
      employeeId: employee.id,
      month,
      year,
      weekendType: shiftConfig.weekendType,
    });
  ...
  const workerEmployees = matchedEmployees.filter((employee) => employee.shiftCategory === "WORKER");
  const workerShiftOptions = shiftConfig.enabledShifts.filter((shift) => shift !== "G");
  if (workerEmployees.length > 0 && workerShiftOptions.length === 0) {
    return {
      error: "Worker employees found but no A/B/C shift is enabled" as const,
      warnings,
...
export async function previewInOutGenerator(clientId: string, month: number, year: number) {
  const loaded = await loadInOutGeneratorInputs(clientId, month, year);
  if (hasLoadError(loaded)) return loaded;
  return {
    daysInMonth: loaded.daysInMonth,
    holidaysCount: loaded.holidays.size,
    holidays: Array.from(loaded.holidays),
    weekendType: loaded.shiftConfig.weekendType,
    shiftConfig: loaded.shiftConfig,
    warnings: loaded.warnings,
